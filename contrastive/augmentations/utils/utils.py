...
def guided_smoothing(image, r=8, e=0.02):
    image_float = np.float32(image) / 255.0

    guidance = image_float

    smoothed_image = cv2.ximgproc.guidedFilter(
        guide=guidance, src=image_float, radius=r, eps=e)

    smoothed_image_8bit = np.uint8(smoothed_image * 255)
    return smoothed_image_8bit
...
if __name__ == '__main__':
    ...
    cv2.imshow('a', img)
    cv2.imshow('b', guided_smoothing(img, 6, 0.01))
    cv2.waitKey()
