...
if os.path.isfile('/utils/library/bi_online_generation.py'):
    from augmentations.utils.library.bi_online_generation import random_get_hull
...
class SBI:
    ...
    def __call__(self, image):
        ...
        '''
        full = image.copy()
        landmark = self.landmarks_detector.dlib_landmarks(image)[0]
        bbox_lm = np.array(
            [landmark[:, 0].min(), landmark[:, 1].min(), landmark[:, 0].max(), landmark[:, 1].max()])
        bboxes = self.landmarks_detector.retina_landmarks(image)[:2]
        ...
        r = img
        f = img.copy()
        r[y0_new: y1_new, x0_new: x1_new] = img_r
        f[y0_new: y1_new, x0_new: x1_new] = img_f
...
if __name__ == '__main__':
    ...
    cv2.imshow('smoothing', guided_smoothing(r))
    cv2.waitKey()
